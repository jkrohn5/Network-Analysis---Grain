...
df2 = pd.read_csv(os.path.join(folderPath, "CornCosts.csv"), dtype=str, skipinitialspace= True, encoding='latin-1')
...
for i in range(0,746):
    start = time.time()
    print("analyzing week " + str(i + 1) + " of 746")
...
#network characteristics for loop
    ...
    facilities = os.path.join(input_gdb,"FeatureDataset", "CornJunctions")
...
#set definition query on county feature class to fips codes of a given week
    ...
    arcpy.management.CalculateField(routes,"Type", "'Corn'", "PYTHON3")
    arcpy.management.CalculateField(facilities,"Type", "'Corn'", "PYTHON3")
    arcpy.management.CalculateField(edges,"Type", "'Corn'", "PYTHON3")
    arcpy.management.CalculateField(spatial,"Type", "'Corn'", "PYTHON3")

    #write data tables to csv
    print("writing routes to csv")
    for attempt in range(3):
        try:
            arcpy.TableToTable_conversion(routes, os.path.join(folderPath, "LoopOutput\Routes"),"Cornroute" + middate.replace("/","_") + ".csv")
        except:
    ...
    for attempt in range(3):
        try:
            arcpy.TableToTable_conversion(facilities, os.path.join(folderPath, "LoopOutput\Facilities"), "Cornfacility" + middate.replace("/","_") + ".csv")
        except:
            print ("This failed because arcpy is dumb, trying again...")
        else:
            break
    ...
    for attempt in range(3):
        try:
            arcpy.TableToTable_conversion(edges, os.path.join(folderPath, "LoopOutput\Edges"),"Cornedge" + middate.replace("/","_") + ".csv")
        except:
            print ("This failed because arcpy is dumb, trying again...")
        else:
            break
    ...
    for attempt in range(3):
        try:
            arcpy.TableToTable_conversion(spatial, os.path.join(folderPath, "LoopOutput\Locks"),"Cornlocks" + middate.replace("/","_") + ".csv")
        except:
            print ("This failed because arcpy is dumb, trying again...")
        else:
            break
    ...
    print("finished week " + str(i + 1) + " of 746")
