...
from [DB].[Schema].JUSTIN_NETWORKSILOS_LOOP a
left JOIN [DB].[Schema].Justin_LoopFacilities b on a.ObjectID = b.ObjectID and left(a.type,4) = left(b.type,4) 
full join [DB].[Schema]L.Justin_LoopRoutes c on b.ObjectID = c.FacilityID and cast(left(b.Analysis_Date,10) as date) = cast(left(c.Analysis_Date,10) as date) and left(b.type,4) = left(c.type,4) 
full join [DB].[Schema].Justin_Edge_Rail_Loop d on c.ObjectID = d.RouteID and cast(left(c.Analysis_Date,10) as date) = cast(d.Analysis_Date as date) and left(c.type,4) = left(d.type,4) 
full join [DB].[Schema].Justin_LoopEdges_River_Loop e on c.ObjectID = e.RouteID and cast(left(c.Analysis_Date,10) as date) = cast(e.Analysis_Date as date) and left(c.type,4) = left(e.type,4)
full join [DB].[Schema].Justin_LoopEdges_Road f on c.ObjectID = f.RouteID  and cast(left(c.Analysis_Date,10) as date) = cast(f.Analysis_Date as date) and left(c.type,4) = left(f.type,4)
left join [DB].[Schema].Justin_Locks_Piovt_Loop  g on cast(c.FacilityID as int)= g.FacilityID and cast(left(c.Analysis_Date,10) as date) = cast(g.Analysis as date) and left(c.type,4) = left(g.type,4)
) a left join
(select instrument, MAX(totalMiles) as TotalMiles from [DB].[Schema].JUSTIN_NETWORKLOOP_BASERESULTS_NEW group by instrument) b on a.instrument = b.instrument
left join
[DB].[Schema].Justin_LoopCosts_Soy c on CAST(left(a.Analysis_Date,10) as date) = CAST(c.Week_of as date)
left join
[DB].[Schema].Justin_LoopCosts_Corn z on CAST(left(a.Analysis_Date,10) as date) = cast(z.Week_of as date)
left join
(select * from [DB].[Schema].Justin_DisasterData where isdate(End_Date) = 1) d on a.FIPS = d.FIPS and cast(left(a.Analysis_Date,10) as date) BETWEEN CAST(d.Begin_Date as datetime) and cast(d.End_Date as datetime)
left join [DB].[Schema].Justin_RiverCounties e on a.FIPS = e.FIPS
